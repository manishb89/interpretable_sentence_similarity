...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="create conceptnet cache images")
    ...
    chunkset = set()
    for s1 in codecs.open('{}/train_2015_10_22.utf-8/STSint.input.images.sent1.chunk.txt'.format(args.data_dir), mode='rt', encoding='utf-8'):
    ...
    for s1 in codecs.open('{}/test/STSint.testinput.images.sent1.chunk.txt'.format(args.data_dir), mode='rt', encoding='utf-8'):
        chunks, bigrams, unigrams = process_sentence(s1, lemmatize=True)
        chunkset.update(chunks, itertools.chain(*bigrams), itertools.chain(*unigrams))

    for s2 in codecs.open('{}/train_2015_10_22.utf-8/STSint.input.images.sent2.chunk.txt'.format(args.data_dir), mode='rt', encoding='utf-8'):
        chunks, bigrams, unigrams = process_sentence(s2, lemmatize=True)
        chunkset.update(chunks, itertools.chain(*bigrams), itertools.chain(*unigrams))
    for s2 in codecs.open('{}/test/STSint.testinput.images.sent2.chunk.txt'.format(args.data_dir), mode='rt', encoding='utf-8'):
        chunks, bigrams, unigrams = process_sentence(s2, lemmatize=True)
        chunkset.update(chunks, itertools.chain(*bigrams), itertools.chain(*unigrams))
    ...
    for rel in rels:
        ...
        with open('{}/cn_images/combined_cn_resource_{}.json'.format(args.data_dir,rel), 'w') as fp:
        ...
        with open('{}/cn_images/combined_cn_resource_{}_processed.json'.format(args.data_dir,rel), 'w') as fp:
            json.dump(cn_resource, fp)
